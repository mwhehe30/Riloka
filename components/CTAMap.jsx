...
import { MapPin, Users, Heart, TrendingUp, Navigation, Landmark } from 'lucide-react';
...
const CTABanjarDetailedMap = () => {
  ...
  const totalStats = {
    ...
    avgRating: umkmData.length > 0 
      ? (umkmData.reduce((sum, umkm) => sum + parseFloat(umkm.rating), 0) / umkmData.length).toFixed(1)
      : '0.0'
  };
          ...
          <div className="relative h-80 sm:h-96 bg-white/5 rounded-2xl p-4 sm:p-6 backdrop-blur-sm border border-white/20 overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-blue-50/5 via-green-50/5 to-purple-50/5">
              ...
              <svg viewBox="0 0 100 100" className="w-full h-full">
                ...
                {/* Green Areas/Parks */}
                <circle cx="25" cy="25" r="3" fill="#22C55E" fillOpacity="0.3" stroke="#22C55E" strokeWidth="0.2" />
                <circle cx="65" cy="35" r="2.5" fill="#22C55E" fillOpacity="0.3" stroke="#22C55E" strokeWidth="0.2" />
                <circle cx="45" cy="65" r="2" fill="#22C55E" fillOpacity="0.3" stroke="#22C55E" strokeWidth="0.2" />

                {/* Landmark Icons */}
                <text x="30" y="60" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">🏪</text>
                <text x="65" y="45" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">🏢</text>
                <text x="40" y="70" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">🍜</text>
                <text x="75" y="75" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">🎨</text>
                <text x="20" y="50" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">🔧</text>
                <text x="55" y="30" fontSize="3" fill="white" fillOpacity="0.6" textAnchor="middle">☕</text>

                {/* Area Labels */}
                <text x="30" y="63" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Pasar</text>
                <text x="65" y="48" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Otto</text>
                <text x="40" y="73" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Soetomo</text>
                <text x="75" y="78" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Hegarsari</text>
                <text x="20" y="53" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Sudirman</text>
                <text x="55" y="33" fontSize="1.5" fill="white" fillOpacity="0.8" textAnchor="middle">Pataruman</text>
              ...
              {banjarAreas.map((area, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <MapPin 
                      size={28} 
                      className={`${getPinColor(area.type)} drop-shadow-lg filter brightness-125`} 
                      fill="currentColor"
                    />
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium w-fit ${
                        banjarAreas[activePin]?.type === 'pasar-tradisional' ? 'bg-orange-500/20 text-orange-300' :
                        banjarAreas[activePin]?.type === 'pusat-bisnis' ? 'bg-blue-500/20 text-blue-300' :
                        banjarAreas[activePin]?.type === 'kuliner' ? 'bg-red-500/20 text-red-300' :
                        banjarAreas[activePin]?.type === 'kerajinan' ? 'bg-purple-500/20 text-purple-300' :
                        banjarAreas[activePin]?.type === 'jasa' ? 'bg-green-500/20 text-green-300' :
                    ...
                    {banjarAreas[activePin]?.umkmList && (
                      <div className="mb-3">
                        <p className="text-white/70 text-sm mb-2">UMKM di area ini:</p>
